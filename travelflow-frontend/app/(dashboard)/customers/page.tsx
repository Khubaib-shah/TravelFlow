...
import { customerDefaultValues, mapCustomerToForm } from "@/features/customers/utils/mapCustomerToForm";
...
export default function CustomersPage() {
  ...
  const onSubmit = async (values: CustomerFormValues) => {
    if (isEditing && editingId) {
      await MockAPI.updateCustomer(editingId, values);
      toast.success("Customer updated successfully");
    } else {
      const customer = await MockAPI.createCustomer(values);
      toast.success("Customer added successfully", { description: `Reference: ${customer.customerRef}` });
    }
    close();
    form.reset(customerDefaultValues);
    await loadData();
  ...
  return (
    <div className="flex flex-col space-y-6">
      ...
      <DrawerForm
        ...
        description={isEditing ? "Update customer profile and contact details." : "Add a new individual or corporate customer to your database."}
        isOpen={isDrawerOpen}
        onClose={close}
        ...
        <Form {...form}>
          <div className="space-y-8">
            <div className="space-y-4">
              <h4 className="text-sm font-semibold text-[var(--tf-text-primary)]">Personal Information</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <FormField control={form.control} name="cnic" label="CNIC" placeholder="XXXXX-XXXXXXX-X" />
                <FormField control={form.control} name="passportNumber" label="Passport" />
              </div>
            </div>

            <div className="space-y-4">
              <h4 className="text-sm font-semibold text-[var(--tf-text-primary)]">Address</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <FormSelect control={form.control} name="city" label="City" required options={PAKISTAN_CITIES.map((c) => ({ label: c, value: c }))} />
            ...
            <div className="space-y-4">
              <h4 className="text-sm font-semibold text-[var(--tf-text-primary)]">Client Type</h4>
              ...
              {customerType === "corporate" && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <FormField control={form.control} name="companyName" label="Company Name" required />
                  <FormSelect control={form.control} name="businessType" label="Business Type" options={[
                  ...
                  <FormField control={form.control} name="taxNumber" label="Tax Number (NTN)" />
                </div>
              )}
            </div>

            <div className="space-y-4">
              <h4 className="text-sm font-semibold text-[var(--tf-text-primary)]">Emergency / Notes</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <FormField control={form.control} name="emergencyContactName" label="Emergency Contact Name" />
                <FormField control={form.control} name="emergencyContactPhone" label="Emergency Contact Phone" type="tel" />
              </div>
