...
import { ArrowLeft, UserPlus, Phone, MapPin, CalendarDays, Wallet, Clock, Tag, ChevronDown, Users } from "lucide-react";
...
export default function LeadDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleStatusChange = async (newStatus: string) => {
    if (!lead) return;
    await MockAPI.updateLead(lead.id, { status: newStatus as Lead["status"] });
    ...
    <div className="space-y-6 pb-12">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="start" className="w-[180px] bg-[var(--tf-surface)] border-[var(--tf-border)]">
                  {["new", "contacted", "follow_up", "interested", "negotiation", "converted", "lost"].map((s) => (
                    <DropdownMenuItem key={s} onClick={() => handleStatusChange(s)} className="capitalize text-sm cursor-pointer">
                      {s.replace("_", " ")}
                    </DropdownMenuItem>
                  ))}
                </DropdownMenuContent>
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" className="bg-[var(--tf-surface)] text-[var(--tf-text-primary)]" onClick={() => setFollowUpOpen(true)}>
            Add Follow-up
          </Button>
          <Button className="bg-[var(--tf-primary)] text-white hover:bg-[var(--tf-primary-hover)]" onClick={() => setConvertOpen(true)}>
            Convert to Booking
          </Button>
        </div>
      </div>
        ...
        <div className="bg-[var(--tf-surface)] rounded-xl border border-[var(--tf-border)] p-6 shadow-sm col-span-1 lg:col-span-2">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
            ...
            <div>
              ...
              <p className="font-medium text-[var(--tf-text-primary)]">{lead.destination}</p>
            </div>
            <div>
              ...
              {lead.budget ? <CurrencyDisplay amount={lead.budget} className="font-medium text-[var(--tf-text-primary)]" /> : <p className="font-medium">Unspecified</p>}
            </div>
            <div>
            ...
            <div>
              <p className="text-xs text-[var(--tf-text-muted)] uppercase tracking-wider mb-1 flex items-center gap-1"><Users className="w-3 h-3" /> Adults / Children</p>
              <p className="font-medium text-[var(--tf-text-primary)]">{lead.adults} / {lead.children}</p>
            </div>
            <div>
              ...
              <p className="font-medium text-[var(--tf-text-primary)] capitalize">{lead.source.replace("_", " ")}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-[var(--tf-surface)] rounded-xl border border-[var(--tf-border)] p-6 shadow-sm">
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center py-2 border-b border-[var(--tf-border)]">
              ...
              <span className="font-medium text-[var(--tf-text-primary)]">{new Date(lead.createdAt).toLocaleDateString("en-GB")}</span>
            </div>
            <div className="flex justify-between items-center py-2 border-b border-[var(--tf-border)]">
              <span className="text-[var(--tf-text-secondary)]">Last Contacted</span>
              <span className="font-medium text-[var(--tf-text-primary)]">
                {lead.lastContactedAt ? new Date(lead.lastContactedAt).toLocaleDateString("en-GB") : new Date(lead.updatedAt).toLocaleDateString("en-GB")}
              </span>
            </div>
            <div className="pt-2">
      ...
      <Tabs defaultValue="activity" className="w-full">
        <TabsList className="bg-[var(--tf-surface)] border border-[var(--tf-border)] p-1 rounded-lg">
          ...
          <TabsTrigger value="followups" className="rounded-md data-[state=active]:bg-[var(--tf-primary)] data-[state=active]:text-white">Follow-ups</TabsTrigger>
        </TabsList>

        <TabsContent value="activity" className="mt-4 bg-[var(--tf-surface)] rounded-xl border border-[var(--tf-border)] p-6">
          <div className="space-y-6">
            {lead.activities.length === 0 ? (
              <p className="text-sm text-[var(--tf-text-muted)]">No activity yet.</p>
            ) : (
              lead.activities.map((activity, index) => (
                <div key={activity.id} className="flex gap-4">
                  <div className="flex flex-col items-center">
                    <div className={`w-3 h-3 rounded-full ${index === 0 ? "bg-[var(--tf-primary)]" : "bg-[var(--tf-border)]"}`} />
                    {index < lead.activities.length - 1 && <div className="w-0.5 h-full bg-[var(--tf-border)] my-1" />}
                  </div>
                  <div className="pb-4 flex-1">
                    <div className="flex justify-between items-start gap-4">
                      <p className="font-medium text-[var(--tf-text-primary)] capitalize">{activity.type.replace("_", " ")}</p>
                      <p className="text-xs text-[var(--tf-text-muted)] flex items-center gap-1 shrink-0">
                        <Clock className="w-3 h-3" /> {formatTimeAgo(new Date(activity.createdAt))}
                      </p>
                    </div>
                    <p className="text-sm text-[var(--tf-text-muted)] mt-1">{activity.description}</p>
                    <p className="text-xs text-[var(--tf-text-muted)] mt-1">by {activity.createdBy}</p>
                  </div>
                </div>
              ))
            )}
          ...
          <div className="max-w-2xl space-y-6">
            <div className="grid grid-cols-2 gap-4">
              <div className="p-4 bg-[var(--tf-surface-2)] rounded-lg">
                ...
                <p className="font-semibold text-lg text-[var(--tf-text-primary)]">{lead.adults}</p>
              </div>
              <div className="p-4 bg-[var(--tf-surface-2)] rounded-lg">
                <p className="text-xs text-[var(--tf-text-muted)] uppercase mb-1">Children</p>
                <p className="font-semibold text-lg text-[var(--tf-text-primary)]">{lead.children}</p>
              </div>
            </div>
            <div>
              <p className="text-sm font-medium text-[var(--tf-text-secondary)] mb-2">Special Requirements</p>
              <p className="text-sm text-[var(--tf-text-primary)]">{lead.specialRequirements || "None specified"}</p>
            </div>
          </div>
        </TabsContent>

        <TabsContent value="followups" className="mt-4 bg-[var(--tf-surface)] rounded-xl border border-[var(--tf-border)] p-6">
          {followUps.length === 0 ? (
            <p className="text-sm text-[var(--tf-text-muted)]">No follow-ups yet.</p>
          ) : (
            <div className="space-y-4">
              {followUps.map((f) => (
                <div key={f.id} className="border-b border-[var(--tf-border)] pb-3 last:border-0">
                  <p className="font-medium capitalize text-[var(--tf-text-primary)]">{f.type.replace("_", " ")}</p>
                  <p className="text-sm text-[var(--tf-text-secondary)] mt-1">{f.description}</p>
                  <p className="text-xs text-[var(--tf-text-muted)] mt-1">{formatTimeAgo(new Date(f.createdAt))} · {f.createdBy}</p>
                </div>
              ))}
            </div>
          )}
      ...
      <FollowUpDrawer lead={lead} isOpen={followUpOpen} onClose={() => setFollowUpOpen(false)} onSaved={loadLead} />
      <ConvertToBookingDrawer lead={lead} isOpen={convertOpen} onClose={() => setConvertOpen(false)} />
    </div>
  );
}
