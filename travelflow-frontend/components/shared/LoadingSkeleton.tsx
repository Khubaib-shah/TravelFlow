  ...
  return (
    <>
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="h-[140px] w-full rounded-xl border border-[var(--tf-border)] bg-[var(--tf-surface)] p-5 shadow-sm">
...
export function DataTableSkeleton({ rows = 5, columns = 5 }: { rows?: number, columns?: number }) {
  return (
    <div className="w-full rounded-lg border border-[var(--tf-border)] bg-[var(--tf-surface)] overflow-hidden">
      <div className="flex items-center gap-4 border-b border-[var(--tf-border)] p-4 bg-[var(--tf-surface-2)]">
        {Array.from({ length: columns }).map((_, i) => (
          <div key={i} className="h-4 flex-1 bg-[var(--tf-border-strong)] rounded animate-pulse opacity-20"></div>
        ))}
      </div>
      {Array.from({ length: rows }).map((_, r) => (
        <div key={r} className="flex items-center gap-4 border-b border-[var(--tf-border)] p-4 last:border-0">
          {Array.from({ length: columns }).map((_, c) => (
            <div key={c} className="h-4 flex-1 bg-[var(--tf-surface-2)] rounded animate-pulse"></div>
          ))}
        </div>
      ))}
    </div>
...
export function ChartSkeleton({ height = 280 }: { height?: number }) {
  return (
    <div 
      className="w-full rounded-xl border border-[var(--tf-border)] bg-[var(--tf-surface)] p-5 shadow-sm flex flex-col gap-4"
