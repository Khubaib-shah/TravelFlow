...
export function SidebarItem({ item, isOpen }: SidebarItemProps) {
  ...
  return (
    ...
    >
      <item.icon className={`h-5 w-5 shrink-0 ${isActive ? "text-[var(--tf-sidebar-active-text)]" : "text-[var(--tf-text-muted)] group-hover:text-[var(--tf-text-primary)] transition-colors"}`} />
      
      {isOpen && (
        <span className="flex-1 truncate tf-body-sm">
          {item.title}
        </span>
      )}
