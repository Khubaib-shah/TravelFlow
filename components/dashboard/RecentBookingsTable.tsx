...
export function RecentBookingsTable({ isLoading: initialLoading }: { isLoading: boolean }) {
  ...
  const columns: ColumnDef<Booking>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => <div className="font-semibold text-[var(--tf-text-primary)]">₨ {row.original.salePrice.toLocaleString()}</div>,
    },
    {
    ...
    },
    {
      id: "actions",
      cell: ({ row }) => (
        <DataTableRowActions row={row} onView={() => console.log(row.original.id)} />
      ),
    },
  ];
  ...
  return (
    <div className="bg-[var(--tf-surface)] border border-[var(--tf-border)] rounded-xl p-6 shadow-sm">
      <div className="flex items-center justify-between mb-6">
        ...
        <button className="text-sm font-medium text-[var(--tf-primary)] hover:text-[var(--tf-primary-hover)]">
          View All
        </button>
      </div>
